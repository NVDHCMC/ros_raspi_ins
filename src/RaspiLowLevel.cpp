...
namespace RASPI {
	...
	void RaspiLowLevel::fetch_data_from_stm32(std::vector<float> * data) {
		...
		for ( i = 0; i < 10; i++ ) {
			if ( i < 7 ) {
				data->at(i) = (float) (((int16_t) this->raw_spi_data[2*i])*256) + ((uint16_t) this->raw_spi_data[2*i + 1]);
			}
			else {
				data->at(i) = ((float) (((int16_t) this->raw_spi_data[2*i + 1])*256) + ((uint16_t) this->raw_spi_data[2*i]));
